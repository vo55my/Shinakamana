...
// Storage untuk cookies (backup only)
const cookieHelpers = {
  setPlaylist(playlist) {
    try {
      // Hanya simpan ID untuk menghemat space
      const animeIds = playlist.map((anime) => anime.mal_id).join(",");
      const expires = new Date(
        Date.now() + 30 * 24 * 60 * 60 * 1000
      ).toUTCString();
      document.cookie = `playlist-ids=${animeIds}; expires=${expires}; path=/; SameSite=Lax`;
      return true;
    } catch (error) {
  ...
  getPlaylist() {
    try {
      const cookie = document.cookie
        .split("; ")
        .find((row) => row.startsWith("playlist-ids="));

      if (cookie) {
        const ids = cookie.split("=")[1].split(",").filter(Boolean);
        return ids.map((id) => ({ mal_id: parseInt(id) }));
      }
      return [];
    } catch {
      return [];
...
export class CompositeStorage {
  constructor() {
    this.storagePriority = ["indexedDB", "localStorage", "cookies"];
  ...
  async initialize() {
    ...
    this.activeStorage = this.supportedStorages[0] || "cookies";
  ...
  testCookies() {
    try {
      document.cookie = "storage-test=1; max-age=60; path=/";
      const hasCookie = document.cookie.includes("storage-test=1");
      document.cookie = "storage-test=1; max-age=0; path=/";
      return hasCookie;
    } catch {
      return false;
    }
      ...
      try {
        ...
        switch (storageType) {
          ...
          case "localStorage":
            ...
            break;
          case "cookies":
            data = cookieHelpers.getPlaylist();
            break;
        }
      ...
      try {
        ...
        switch (storageType) {
          ...
          case "localStorage":
            ...
            break;
          case "cookies":
            result = cookieHelpers.setPlaylist(playlist);
            break;
        }
    ...
    for (const storageType of this.supportedStorages) {
      try {
        switch (storageType) {
          ...
          case "cookies":
            document.cookie =
              "playlist-ids=; expires=Thu, 01 Jan 1970 00:00:00 GMT; path=/";
            break;
        }
